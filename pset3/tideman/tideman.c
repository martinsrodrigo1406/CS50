...
void record_preferences(int ranks[])
{
    for(int i = 0; i < candidate_count; i++)
    {
        ...
        {
            bool prefered = false;
            for(int n = i; n > 0; n--) //loop para contar candidatos que já foram computados
            {
                    if(j == ranks[n - 1]) //Se candidato J já apareceu em posição anterior no rank
                    {
                        prefered = true;
                    }
            }
            if(!prefered) //se cadidato J não foi preferido ainda, ponto para candidato I
            {
                if(ranks[i] != j) //não computar novo voto sobre si mesmo
                {
